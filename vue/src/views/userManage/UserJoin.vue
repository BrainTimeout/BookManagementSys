<template>
  <div class="user-join-container">
    <el-form :model="userForm" ref="userForm" label-width="120px" class="user-form">
      ...
      <!-- 账户类型 -->
      <el-form-item label="账户类型" prop="usertype" :rules="[{ required: true, message: '请选择账户类型', trigger: 'blur' }]">
      ...
      <el-form-item label="简介" prop="introduce">
        <el-input v-model="userForm.introduce" type="textarea" placeholder="请输入简介" />
      </el-form-item>
  ...
  methods: {
    ...
    async handleSubmit() {
      try {
        ...
        // 提交数据
        const res = await request.post("/Auth/Join", joinRequest);
        if (res.code === "200") {
